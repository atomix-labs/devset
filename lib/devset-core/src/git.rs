//! Git sources, read through the user's own `git` into a bare repository per URL.

use alloc::collections::BTreeMap;
use core::str;
use std::io::{self, BufRead, BufReader, Read, Write};
use std::process::{Command, Stdio};

use camino::{Utf8Path, Utf8PathBuf};

use crate::digest::Digest;
use crate::errors::{Error, Result, SourceError};
use crate::path::RelPath;
use crate::source::{Cache, Fetch, GitRef, Oid};
use crate::tree::Tree;

/// A commit in a cached bare repository, with the files under the profile's directory.
#[derive(Debug)]
pub(crate) struct Commit {
    /// The bare repository.
    repo: Utf8PathBuf,
    /// The commit.
    rev: Oid,
    /// Files under the profile's directory, by path relative to it.
    files: BTreeMap<String, Blob>,
}

/// A file in a commit.
#[derive(Debug)]
struct Blob {
    /// Object id.
    id: String,
    /// Whether it is a regular file rather than a symlink or submodule.
    regular: bool,
}

impl Commit {
    /// The commit of `url` at `pin`, or at `at` resolved now, fetched into `cache` if absent.
    ///
    /// `shown` is the URL as configured, for progress.
    pub(crate) fn open(
        url: &str, shown: &str, at: &GitRef, dir: Option<&RelPath>, pin: Option<&Oid>,
        cache: &Cache,
    ) -> Result<Self> {
        let repo = cache.path().join("git").join(Digest::of(url.as_bytes()).to_string());
        if !repo.join("HEAD").is_file() {
            fs_err::create_dir_all(&repo)?;
            git(Some(&repo), &["init", "--bare", "-q"])?;
        }
        let mut remote = false;
        let mut contact = || {
            if !remote {
                cache.notify(Fetch::Start(shown));
                remote = true;
            }
        };
        let rev = match (pin, at) {
            (Some(rev), _) | (None, GitRef::Rev(rev)) => rev.clone(),
            (None, _) => {
                if let Some(rev) = cache.resolved(url, &wanted(at)) {
                    rev
                } else {
                    contact();
                    let rev = resolve(url, at)?;
                    cache.remember(url, &wanted(at), &rev);
                    rev
                }
            },
        };
        if !has(&repo, &rev)? {
            contact();
            fetch(&repo, url, at, &rev)?;
        }
        if remote {
            cache.notify(Fetch::Done(shown));
        }
        // `<rev>:<dir>` names the subtree literally; path arguments would be glob patterns.
        let tree = dir.map_or_else(|| format!("{rev}^{{tree}}"), |dir| format!("{rev}:{dir}"));
        let listing = git(Some(&repo), &["ls-tree", "-r", "-z", &tree])?;
        Ok(Self { repo, rev, files: parse_listing(&listing) })
    }

    /// The commit.
    pub(crate) const fn rev(&self) -> &Oid {
        &self.rev
    }

    /// [`Reader::read`](crate::source::Reader::read) for this commit.
    pub(crate) fn read(&self, dir: &str, paths: &[RelPath]) -> Result<Tree> {
        let mut wanted = Vec::with_capacity(paths.len());
        for path in paths {
            let key =
                if dir.is_empty() { path.as_str().to_owned() } else { format!("{dir}/{path}") };
            match self.files.get(&key) {
                None => {},
                Some(blob) if blob.regular => wanted.push((path, blob.id.as_str())),
                Some(_) => {
                    return Err(
                        SourceError::NotAFile { file: format!("{}:{key}", self.rev) }.into()
                    );
                },
            }
        }
        let mut tree = Tree::default();
        if wanted.is_empty() {
            return Ok(tree);
        }
        // Lockstep, one object per request: `--batch` flushes each reply, so this cannot
        // deadlock on full pipes however many files there are.
        let mut child = command(Some(&self.repo), &["cat-file", "--batch"])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
            .map_err(spawn_error)?;
        let mut stdin = child.stdin.take().ok_or_else(|| io::Error::other("git: no stdin"))?;
        let stdout = child.stdout.take().ok_or_else(|| io::Error::other("git: no stdout"))?;
        let mut stdout = BufReader::new(stdout);
        let mut header = String::new();
        for (path, id) in wanted {
            writeln!(stdin, "{id}")?;
            header.clear();
            stdout.read_line(&mut header)?;
            let size = blob_size(&header)
                .ok_or_else(|| io::Error::other(format!("git cat-file: {}", header.trim_end())))?;
            tree.insert(path.clone(), |buf| {
                let read = stdout.by_ref().take(size).read_to_end(buf)?;
                if u64::try_from(read).ok() == Some(size) {
                    Ok(())
                } else {
                    Err(io::ErrorKind::UnexpectedEof.into())
                }
            })?;
            stdout.read_exact(&mut [0])?;
        }
        drop(stdin);
        let status = child.wait()?;
        if !status.success() {
            return Err(failed("cat-file --batch".to_owned(), status.to_string()));
        }
        Ok(tree)
    }
}

/// The size in a `cat-file --batch` header for a blob: `<id> blob <size>`.
fn blob_size(header: &str) -> Option<u64> {
    let mut fields = header.trim_end().split(' ');
    let (_, kind, size) = (fields.next()?, fields.next()?, fields.next()?);
    (kind == "blob").then(|| size.parse().ok()).flatten()
}

/// `ls-tree -r -z` output, by path.
fn parse_listing(raw: &[u8]) -> BTreeMap<String, Blob> {
    raw.split(|&b| b == 0)
        .filter_map(|entry| {
            let (meta, path) = str::from_utf8(entry).ok()?.split_once('\t')?;
            let mut meta = meta.split(' ');
            let (mode, kind, id) = (meta.next()?, meta.next()?, meta.next()?);
            let regular = kind == "blob" && matches!(mode, "100644" | "100755");
            Some((path.to_owned(), Blob { id: id.to_owned(), regular }))
        })
        .collect()
}

/// What to ask the remote for: a full ref name, or the commit id itself.
fn wanted(at: &GitRef) -> String {
    match at {
        GitRef::Head => "HEAD".to_owned(),
        GitRef::Branch(branch) => format!("refs/heads/{branch}"),
        GitRef::Tag(tag) => format!("refs/tags/{tag}"),
        GitRef::Rev(rev) => rev.to_string(),
    }
}

/// The commit `at` names on the remote now.
fn resolve(url: &str, at: &GitRef) -> Result<Oid> {
    if let GitRef::Rev(rev) = at {
        return Ok(rev.clone());
    }
    let name = wanted(at);
    let peeled = format!("{name}^{{}}");
    let listing = git(None, &["ls-remote", "--", url, &name, &peeled])?;
    let listing = String::from_utf8_lossy(&listing);
    let mut found = None;
    for (id, reference) in listing.lines().filter_map(|line| line.split_once('\t')) {
        if reference == peeled {
            found = Some(id);
            break;
        }
        if reference == name {
            found = Some(id);
        }
    }
    let command = || format!("ls-remote {url}");
    let id = found.ok_or_else(|| failed(command(), format!("no {name}")))?;
    Oid::try_from(id.to_owned()).map_err(|e| failed(command(), e.to_string()))
}

/// Whether `repo` has commit `rev`.
fn has(repo: &Utf8Path, rev: &Oid) -> Result<bool> {
    let object = format!("{rev}^{{commit}}");
    let status = command(Some(repo), &["cat-file", "-e", &object])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()
        .map_err(spawn_error)?;
    Ok(status.success())
}

/// Fetches `rev` into `repo`: by id, or through `at`'s ref from a server that refuses ids.
fn fetch(repo: &Utf8Path, url: &str, at: &GitRef, rev: &Oid) -> Result<()> {
    let by_id =
        git(Some(repo), &["fetch", "-q", "--depth", "1", "--no-tags", "--", url, rev.as_str()]);
    let Err(refused) = by_id else { return Ok(()) };
    if matches!(at, GitRef::Rev(_)) {
        return Err(refused);
    }
    git(Some(repo), &["fetch", "-q", "--no-tags", "--", url, &wanted(at)])?;
    if has(repo, rev)? { Ok(()) } else { Err(refused) }
}

/// A `git` invocation, in `repo` if given.
fn command(repo: Option<&Utf8Path>, args: &[&str]) -> Command {
    let mut command = Command::new("git");
    if let Some(repo) = repo {
        command.arg("-C").arg(repo);
    }
    command.args(args);
    command
}

/// Runs `git`, returning its output.
fn git(repo: Option<&Utf8Path>, args: &[&str]) -> Result<Vec<u8>> {
    let output = command(repo, args).stdin(Stdio::null()).output().map_err(spawn_error)?;
    if output.status.success() {
        Ok(output.stdout)
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_owned();
        Err(failed(args.join(" "), stderr))
    }
}

/// `command` failing with `stderr`.
fn failed(command: String, stderr: String) -> Error {
    SourceError::Git { command, stderr }.into()
}

/// A failure to start `git`.
fn spawn_error(error: io::Error) -> Error {
    if error.kind() == io::ErrorKind::NotFound { SourceError::NoGit.into() } else { error.into() }
}
